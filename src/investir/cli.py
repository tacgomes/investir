...
from investir.exceptions import InvestirError, SkippableError
...
def abort(msg: Exception | str) -> None:
    logger.critical(str(msg))
    if isinstance(msg, SkippableError):
        option = typer.style("--no-strict", fg=typer.colors.CYAN, bold=True)
        print(f"\nUse the {option} option if you want to ignore this error.")
