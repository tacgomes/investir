...
def test_section_104_disposal(make_tax_calculator):
    ...
    assert cg.cost == Decimal("600.00")
    assert cg.gain_loss == Decimal("3400.00")
    assert str(cg) == (
    ...
    assert holding.cost == Decimal("2400.00")
...
def test_orders_not_realised_in_pound_sterling(make_tax_calculator):
    order1 = Acquisition(
        datetime(2015, 4, 1, tzinfo=timezone.utc),
        isin=ISIN("X"),
...
#######################################################################
# Tests for calculation examples provided online
#######################################################################
